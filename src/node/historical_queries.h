...
namespace ccf::historical
{
  ...
  protected:
    ...
    void bar(size_t something)
    {
      // Always erase if present.
      // Put back in the beginning with 50% chance.
      auto it = std::find(foo.begin(), foo.end(), something);
      if (it != foo.end())
      {
        foo.erase(it);
      }
      if (rand() % 2)
      {
        foo.insert(foo.begin(), something);
      }
    }
