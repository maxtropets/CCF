...
namespace ccf::historical
{
  ...
  protected:
    ...
    {
      // LOG_INFO_FMT("add ref seq {} handle {} start", seq, handle);
      ...
      // LOG_INFO_FMT("add ref seq {} handle {} end", seq, handle);
    }

    void add_refs(CompoundHandle handle)
    {
      // const auto seqs_it = requests.find(handle);
      // assert(seqs_it != requests.end());
      // for (auto it = seqs_it->second.my_stores.begin();
      //      it != seqs_it->second.my_stores.end();
      //      ++it)
      // {
      //   add_ref(it->first, handle);
      // }
    }

    void remove_ref(SeqNo seq, CompoundHandle handle)
    {
      // LOG_INFO_FMT("rem ref seq {} handle {} start", seq, handle);
      ...
      // LOG_INFO_FMT("rem ref seq {} handle {} end", seq, handle);
    }
