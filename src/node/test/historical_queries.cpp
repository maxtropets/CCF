...
TEST_CASE("StateCache point queries")
{
  ...
  [[maybe_unused]] static const ccf::historical::RequestHandle retry_handle = 3;
...
TEST_CASE("StateCache get store vs get state")
{
...

  {
    INFO("Switching between range store requests and range state requests");
    {
      ...
      for (auto& historical_state : states)
      {
...
TEST_CASE("StateCache range queries")
{
  ...
  [[maybe_unused]] auto signing_version =
    [&signature_versions](ccf::kv::Version seqno) {
...
TEST_CASE("StateCache concurrent access")
{
  auto state = create_and_init_state();
  auto& kv_store = *state.kv_store;
  [[maybe_unused]] const auto default_handle = 0;
  ...
  auto validate_all_states =
    [&](const std::vector<ccf::historical::StatePtr>& states) {
      ...
      {
        REQUIRE(historical_state != nullptr);
        const auto seqno = historical_state->store->current_txid().seqno;
        if (
          std::find(
            signature_versions.begin(), signature_versions.end(), seqno) ==
          signature_versions.end())
        {
          validate_business_transaction(historical_state, seqno);
        }
      }
    };
  ...
  [[maybe_unused]] auto query_random_point_state =
    [&](ccf::SeqNo target_seqno, size_t handle, const auto& error_printer) {
      ccf::historical::StatePtr historical_state;
      auto fetch_result = [&]() {
        historical_state = cache.get_state_at(handle, target_seqno);
      };
      auto check_result = [&]() { return historical_state != nullptr; };
      REQUIRE(fetch_until_timeout(fetch_result, check_result, error_printer));
      REQUIRE(historical_state != nullptr);
      validate_all_states({historical_state});
    };
    ...
    for (size_t i = 0; i < per_thread_queries; ++i)
    {
      ...
      switch (query_kind)
      {
        ...
        case 2:
        {
          ...
          for (auto seqno = range_start; seqno != range_end; ++seqno)
          {
            if (seqno % 3 != 0)
            {
              seqnos.insert(seqno);
