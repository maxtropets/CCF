...
TEST_CASE("create_service never drops a published signing identity")
{
  ccf::kv::Store store;
  ...
  const auto service_cert = service_key->self_sign(
    ...
    valid_from,
    ccf::crypto::compute_cert_valid_to_string(valid_from, 1));
  const ccf::Identity pq_identity{
    ccf::IdentityKind::X509_SPKI_DER, std::vector<uint8_t>{1, 2, 3}};
  const ccf::Identity next_pq_identity{
    ccf::IdentityKind::X509_SPKI_DER, std::vector<uint8_t>{4, 5, 6}};

  {
    ...
    InternalTablesAccess::create_service(
      ...
      nullptr,
      false,
      {{ccf::IdentityType::PQ, pq_identity}});
    REQUIRE(tx.commit() == ccf::kv::CommitResult::SUCCESS);
  }

  {
    ...
    tx.wo<ccf::SerialisedMerkleTree>(Tables::SERIALISED_MERKLE_TREE)
      ->put(tree.serialise());
    REQUIRE_THROWS(InternalTablesAccess::create_service(
      tx, service_cert, {2, 10}, nullptr, true, {}));
  }

  {
    ...
    InternalTablesAccess::create_service(
      ...
      nullptr,
      true,
      {{ccf::IdentityType::PQ, next_pq_identity}});
    REQUIRE(tx.commit() == ccf::kv::CommitResult::SUCCESS);
  }

  auto tx = store.create_read_only_tx();
  auto* identities = tx.ro<ccf::SigningIdentities>(Tables::SIGNING_IDENTITIES);
  REQUIRE(identities->get(ccf::IdentityType::PQ) == next_pq_identity);
  REQUIRE(
    identities->get(ccf::IdentityType::CLASSICAL)->value ==
    service_key->public_key_der());
}
