...
TEST_CASE("get_version_of_previous_write")
{
...

  {
...

    {
      ...
      auto other_handle = tx_other.rw(map);
      other_handle->put(k2, v3);
      tx_other.commit();
    }
...
TEST_CASE("size")
{
...

  {
...

    {
      ...
      auto tx2 = kv_store.create_tx();
      auto tx2_handle = tx2.rw(map);

      REQUIRE(tx2_handle->size() == 2);
      tx2_handle->remove(k2);
      REQUIRE(tx2_handle->size() == 1);
    }
...
TEST_CASE("Deserialising from other Store")
{
  ...
  REQUIRE(
    clone.deserialize(serialised_data)->apply() == ccf::kv::ApplyResult::PASS);
}
...
TEST_CASE("Reported TxID after commit")
{
  ...
  INFO("Simple read-only tx");
  {
    ...
    auto tx = kv_store.create_tx();
    (void)tx.ro(map); // Remember: opacity tx_id is acquired here
