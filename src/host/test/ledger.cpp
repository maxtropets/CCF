...
TEST_CASE("Recovery resilience")
{
  ...
  SUBCASE("Corrupt table offset in committed chunk has no effect")
  {
    ...
    const auto recovered_last_idx = new_ledger.get_last_idx();
    TestEntrySubmitter recovered_entry_submitter(
      new_ledger, chunk_threshold, recovered_last_idx);
    recovered_entry_submitter.write(true);
    REQUIRE(recovered_entry_submitter.get_last_idx() == recovered_last_idx + 1);
  }
  ...
  SUBCASE("Corrupt last entry")
  {
    ...
    entry_submitter.write(true);
    entry_submitter.write(true);
    size_t last_entry_idx = entry_submitter.get_last_idx();
    ...
    REQUIRE(new_ledger.get_last_idx() == last_entry_idx - 1);
    REQUIRE(number_of_files_in_ledger_dir() == 2);
